// oj.Typeahead - A Typeahead.js plugin for OJ.

// The following the beta of typeahead 0.10.0
// http://twitter.github.io/typeahead.js/beta/releases/latest/typeahead.bundle.js
...
;(function(root, factory){
  ...
  var plugin = function(oj,settings){
    ...
    var Typeahead = oj.createType('Typeahead', {
      ...
      // Typeahead('Initial value', {placeholder:'Placeholder text', data:["complete","this"]})
      constructor: function(){
        ...
        // Write them only once since typeahead.js cannot reinitialize values
        props = {
          highlight: true,
          minLength: 1,
          menuWidth: 'auto',
        ...
        for(key in props) {
          var value = oj.argumentShift(options, key)
          if(value == null)
            value = props[key]
          _t['_' + key] = value
        }

        Typeahead.base.constructor.apply(this, [options]);
        ...
        // Bind events to handle change
        $input.bind({
          keyup: function(){
            if (_t.live)
              setTimeout((function(){return $input.change()}), 10)
          },
      ...
      properties: {
        ...
        value: {
          ...
          set: function(v){
            if(!this.isConstructed)
              return
            this.$input.get(0).value = v
          }
        },
        highlight: {get: function(){return this._highlight}},
        minLength: {get: function(){return this._minLength}},
        menuWidth: {get:function(){return this._menuWidth}},

        // Define all sections at once using an array of Section objects
        sections: {get: function(){return this._sections}},

        // Define the source (dataset) in parts
        name: {get: function(){return this._name}},             // name of data source
        valueKey: {get: function(){return this._valueKey}},     // key to lookup values with
        limit: {get: function(){return this._limit}},           // max number of suggestions
        local: {get: function(){return this._local}},           // local data
        remote: {get: function(){return this._remote}},         // url or Remote options hash
        prefetch: {get: function(){return this._prefetch}},     // url or Prefetch options hash
        tokenizer: {get: function(){return this._tokenizer}},   // function to split str into tokens
        dupChecker: {get: function(){return this._dupChecker}}, // function (d1,d2) returns true if duplicate
        sorter: {get: function(){return this._sorter}},         // compare function to sort suggestions

        // Templating options
        header: {get: function(){return this._header}},
        footer: {get: function(){return this._footer}},
      ...
      methods: {
        ...
        // Initialize the jquery typeahead plugin when inserted
        inserted:function(){
          var _t = this,
            $input = _t.$el,

          // Remove themes and add them to new root
            themes  = _t.themes

          $input.removeClass('oj-Typeahead')

          _t.themes = []

          // Initialize typeahead plugin
          var options = {
            minLength:_t.minLength,
            highlight:_t.highlight
          }
          // Initialize with many sections at once
          if (this.sections) {
            $input.typeahead.apply($input, [options].concat(_t.sections))
          // Or initialize with one section defined by properties
          } else {
            $input.typeahead(options,{
              // Create implicit source section for a more natural syntax
              source:{
                name: _t.name,
                valueKey: _t.valueKey,
                limit: _t.limit,
                local: _t.local,
                remote: _t.remote,
                prefetch: _t.prefetch,
                tokenizer: _t.tokenizer,
                dupChecker: _t.dupChecker,
                sorter: _t.sorter
              },
              // Convert templates to use OJ templating instead of mustache
              templates:{
                suggestion:_t._convertTemplate(_t.suggestion),
                empty:_t._convertTemplate(_t.empty),
                header:_t._convertTemplate(_t.header),
                footer:_t._convertTemplate(_t.footer)
              }
            })
          }

          var $root = $input.parent(),
            $menu = $root.find('.tt-dropdown-menu'),
            $hint = $root.find('.tt-hint')

          if (_t.menuWidth == 'auto') {
            $input.on('typeahead:opened',function(){
              $menu.css('width',_t.$input.outerWidth() + 'px');
            });
          }
          else if(_t.menuWidth)
            $menu.css('width', _t.menuWidth);

          // Typeahead just generated a bunch of stuff.
          // Add oj-Typeahead class to new root
          var classes = $root.attr('class')
          $root.attr('class', 'oj-Typeahead ' + classes)

          // On the `<input>` element rename oj-Typeahead to oj-Typeahead-input
          $root.find('oj-Typeahead').each(function(el){
             classes = $(el).attr('class')
            $(el).attr('class', 'oj-Typeahead-input ' + classes)
          })

          // Reset el to root to include Typeahead generated stuff in theming
          _t._el = $root.get(0)
          _t._$el = null
          _t._el.oj = _t

          // Add themes back now that the root is reset
          _t.themes = themes

          // Add `oj-Typeahead-hint` class
          classes = $hint.attr('class')
          $hint.attr('class', 'oj-Typeahead-hint ' + classes)

          // Add 'oj-Typeahead-input' class
          classes = $input.attr('class')
          $input.attr('class', 'oj-Typeahead-input ' + classes)

          // Add `oj-Typeahead-menu` class
          classes = $menu.attr('class')
          $menu.attr('class', 'oj-Typeahead-menu ' + classes)
        },
        _convertTemplate:function(template){
          if (template)
            return function(data){return oj.toHTML(function(){return template(data)})}
          else
            return null
        }
      }
    ...
    Typeahead.theme('bootstrap3', {
      ...
      '.tt-suggestion':{
        ...
        '&.tt-cursor':{
          ...
          backgroundImage: 'linear-gradient(to bottom, #0088cc, #0077b3)',
